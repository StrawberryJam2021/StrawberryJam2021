    ...
    public class InstantTeleportTrigger : Trigger {
        ...
        public override void OnEnter(Player player) {
            if (onExit) {
                ...
                if (delay > 0) {
                    Add(new Coroutine(OnEnterSequence(player)));
                } else if (!triggered && StrawberryJam2021Module.VivHelperGetFlags(level, flags, "and") && player != null && !player.Dead) {
                    Teleport(player);
                }
            }
                ...
                level.OnEndOfFrame += delegate {
                    ...
                    foreach (Follower follower in leader.Followers) {
                        if (follower.Entity != null) {
                            ...
                            if (!CompareEntityIDs(follower.ParentEntityID, EntityID.None)) {
                    ...
                    if (newPos.X >= 0 && newPos.X <= level.Bounds.X + level.Bounds.Width - level.LevelOffset.X &&
                        ...
                        if (legacyCamera == 2) {
                            ...
                            CameraShit(level, player, level.LevelOffset + vals[1]);
                        }
