        ...
        private static void LightingRenderer_Render(On.Celeste.LightingRenderer.orig_Render orig, LightingRenderer self, Scene scene) {
            ...
            if (scene is Level level && lightingMasks.Count > 0) {
                ...
                if (FadeBuffers.Count > fadeMasks.Length) {
                    for (int i = fadeMasks.Length; i < FadeBuffers.Count; i++) {
                        FadeBuffers[i].Dispose();
                    }

                    FadeBuffers.RemoveRange(fadeMasks.Length, FadeBuffers.Count - fadeMasks.Length);
                } else {
                    for (int i = FadeBuffers.Count; i < fadeMasks.Length; i++) {
                ...
                if (fadeMasks.Length > 0) {
                    ...
                    for (int i = 0; i < fadeMasks.Length; i++) {
                        var mask = fadeMasks[i];
                        mask.BufferIndex = i;

                        float lightingTo = (mask.LightingTo >= 0f ? mask.LightingTo : level.Session.LightingAlphaAdd);
                        float lightingFrom = (mask.LightingFrom >= 0f ? mask.LightingFrom : level.Session.LightingAlphaAdd);
                        ...
                        bool inverted = lightingTo > lightingFrom;
                        float baseAlpha = inverted ? (1f - lightingFrom) : lightingFrom;
                        float subAlpha = lightingFrom - lightingTo;
                ...
                foreach (LightingMask mask in lightingMasks) {
                    float lightingTo = (mask.LightingTo >= 0f ? ((mask.AddBase ? level.BaseLightingAlpha : 0f) + mask.LightingTo) : level.BaseLightingAlpha + level.Session.LightingAlphaAdd);
                    float lightingFrom = (mask.LightingFrom >= 0f ? ((mask.AddBase ? level.BaseLightingAlpha : 0f) + mask.LightingFrom) : level.BaseLightingAlpha + level.Session.LightingAlphaAdd);

                    foreach (var slice in mask.GetMaskSlices()) {
                        float lighting = MathHelper.Clamp(slice.GetValue(lightingFrom, lightingTo), 0f, 1f);
