...
namespace Celeste.Mod.StrawberryJam2021.Entities {
    ...
    class SwitchCrate : Actor {
        ...
        public SwitchCrate(Vector2 position)
            : base(position) {
        ...
        public SwitchCrate(EntityData data, Vector2 offset)
        ...
        public override void Update() {
            ...
            if (Hold.IsHeld) {
                ...
                if (OnGround()) {
                    if(OnFloor()) {
                        CurrentFloorTime++;
                        if(CurrentFloorTime >= TimeToExplode) {
                            Die();
                        }
        ...
        public bool OnFloor(int downCheck = 1) {
            Type type = Everest.Modules.First(m => m.Metadata.Name == "FactoryHelper").GetType("Conveyer");
            if (CollideCheck<type>(Position + Vector2.UnitY * downCheck)) {
